...
function saveDay(day,films,activities){const all=loadAll();all[day]={films,activities};saveAll(all)}
function loadDay(day){return loadAll()[day]||{films:[],activities:[]}}
function allSaved(){const all=loadAll(),films=[],activities=[];days.forEach(d=>{const x=all[d]||{};(x.films||[]).forEach(v=>films.push(v));(x.activities||[]).forEach(v=>activities.push(v))});return{films,activities}}
...
function saveCurrentDay(){if(selected==="全部")return;const d=currentSelection();saveDay(selected,d.films,d.activities)}
function currentSelection(){const d=loadDay(selected);return{films:d.films||[],activities:d.activities||[]}}
function selectedMaps(){const d=selected==="全部"?allSaved():currentSelection();return{films:new Set((d.films||[]).map(x=>x.key)),activities:new Set((d.activities||[]).map(x=>x.key))}}
...
function render(){renderDays();refreshTop();const maps=selectedMaps();const raw=(selected==="全部"?SCHEDULE:SCHEDULE.filter(x=>x[0]===selected)).slice().sort((a,b)=>dayIndex(a[0])-dayIndex(b[0])||a[2].localeCompare(b[2]));const acts=(selected==="全部"?MASTER_ACTIVITIES:MASTER_ACTIVITIES.filter(x=>x.date===selected)).slice().sort((a,b)=>dayIndex(a.date)-dayIndex(b.date)||a.start.localeCompare(b.start));const timeline=[];raw.forEach(x=>timeline.push({type:"film",start:x[2].split("-")[0],date:x[0],item:x}));acts.forEach(x=>timeline.push({type:"activity",start:x.start,date:x.date,item:x}));timeline.sort((a,b)=>dayIndex(a.date)-dayIndex(b.date)||a.start.localeCompare(b.start)||(a.type==="activity"?-1:1));timelineEl.innerHTML=timeline.map(x=>x.type==="film"?filmHtml(x.item,maps.films):activityHtml(x.item,maps.activities)).join("");bindTimeline();const d=selected==="全部"?allSaved():currentSelection();$("#selectedCount").textContent=d.films.length;$("#activityCountText").textContent=d.activities.length?` + ${d.activities.length} 场大师班`:""}
...
function bindTimeline(){timelineEl.querySelectorAll("[data-action]").forEach(el=>el.onclick=()=>{const action=el.dataset.action,key=el.dataset.key;if(action==="toggleFilm")toggleFilmByKey(key);else if(action==="toggleActivity")toggleActivityByKey(key);else if(action==="openFilm")openFilmByKey(key);else if(action==="openActivity")openActivityByKey(key);else if(action==="ticket")goTicketByKey(key);else if(action==="closeDetail"){detailKey="";render()}})}
function findFilm(key){return SCHEDULE.find(x=>keyOf(x)===key)}function findAct(key){return MASTER_ACTIVITIES.find(x=>activityKey(x)===key)}
function toggleFilmByKey(key){const item=findFilm(key);if(!item)return;if(selected==="全部"){const d=loadDay(item[0]),arr=(d.films||[]).slice(),i=arr.findIndex(x=>x.key===key);if(i>=0)arr.splice(i,1);else arr.push({key,item});saveDay(item[0],arr,d.activities||[])}else{const d=currentSelection(),arr=d.films.slice(),i=arr.findIndex(x=>x.key===key);if(i>=0)arr.splice(i,1);else arr.push({key,item});arr.sort((a,b)=>a.item[2].localeCompare(b.item[2]));saveDay(selected,arr,d.activities)}detailKey="";render()}
function toggleActivityByKey(key){const item=findAct(key);if(!item)return;if(selected==="全部"){const d=loadDay(item.date),arr=(d.activities||[]).slice(),i=arr.findIndex(x=>x.key===key);if(i>=0)arr.splice(i,1);else arr.push({key,item});saveDay(item.date,d.films||[],arr)}else{const d=currentSelection(),arr=d.activities.slice(),i=arr.findIndex(x=>x.key===key);if(i>=0)arr.splice(i,1);else arr.push({key,item});arr.sort((a,b)=>a.item.start.localeCompare(b.item.start));saveDay(selected,d.films,arr)}detailKey="";render()}
...
function entriesForMode(mode){const entries=[];if(mode==="full"||mode==="all"){const all=loadAll();days.forEach(day=>{const d=all[day]||{};(d.activities||[]).forEach(x=>entries.push({date:day,weekday:x.item.weekday,type:"activity",start:x.item.start,end:x.item.end,title:x.item.title,en:x.item.en,venue:x.item.venue,item:x.item}));(d.films||[]).forEach(x=>entries.push({date:day,weekday:x.item[1],type:"film",start:x.item[2].split("-")[0],end:x.item[2].split("-")[1],title:x.item[4],en:x.item[5],venue:x.item[3],unit:x.item[9],status:x.item[6],item:x.item}))})}else{const d=currentSelection();d.activities.forEach(x=>entries.push({date:selected,weekday:x.item.weekday,type:"activity",start:x.item.start,end:x.item.end,title:x.item.title,en:x.item.en,venue:x.item.venue,item:x.item}));d.films.forEach(x=>entries.push({date:selected,weekday:x.item[1],type:"film",start:x.item[2].split("-")[0],end:x.item[2].split("-")[1],title:x.item[4],en:x.item[5],venue:x.item[3],unit:x.item[9],status:x.item[6],item:x.item}))}entries.sort((a,b)=>dayIndex(a.date)-dayIndex(b.date)||a.start.localeCompare(b.start));return entries}
function openCalendar(mode){calendarMode=mode;const entries=entriesForMode(mode);if(!entries.length){showToast(mode==="full"?"还没有选择任何观影安排":"先选择今天的电影或大师班");return}const filmCount=entries.filter(x=>x.type==="film").length;const activityCount=entries.filter(x=>x.type==="activity").length;$("#calendarTitle").textContent=mode==="full"?"全程观影日历":selected+" · 我的观影日历";$("#calendarSub").textContent="确认后保存一张你的观影日程图片";$("#calendarConfirmText").textContent=mode==="full"?`9/24—9/30 · ${filmCount} 场电影 + ${activityCount} 场大师班`:`${filmCount} 场电影 + ${activityCount} 场大师班`;$("#ready").textContent="正在生成图片……";$("#saveImage").disabled=true;openModal("calendarModal");setTimeout(async()=>{try{await drawShareImage(entries,mode);$("#saveImage").disabled=false;$("#ready").textContent="图片已生成，点击“保存图片”即可保存"}catch(e){$("#ready").textContent="图片生成失败，请返回后重试"}},80)}
function previewHtml(entries,mode){let last="";return entries.map(x=>{let h="";if(mode==="full"&&x.date!==last){last=x.date;h+=`<div class="smartSectionTitle">${x.date} <small>${escapeHtml(x.weekday||"")}</small></div>`}h+=`<div class="calItem ${x.type==="activity"?"calActivity":""}"><span class="calTime">${x.start}—${x.end}</span><span>${escapeHtml(x.title)}</span><span class="calHall">${escapeHtml(x.type==="activity"?"大师·对话":x.venue)}</span></div>`;return h}).join("")}
