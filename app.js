...
function entriesForMode(mode){const entries=[];if(mode==="full"||mode==="all"){const all=loadAll();days.forEach(day=>{const d=all[day]||{};(d.activities||[]).forEach(x=>{const fresh=findAct(x.key)||x.item;entries.push({date:day,weekday:fresh.weekday,type:"activity",start:fresh.start,end:fresh.end,title:fresh.title,en:fresh.en,venue:fresh.venue,item:fresh})});(d.films||[]).forEach(x=>entries.push({date:day,weekday:x.item[1],type:"film",start:x.item[2].split("-")[0],end:x.item[2].split("-")[1],title:x.item[4],en:x.item[5],venue:x.item[3],unit:x.item[9],status:x.item[6],item:x.item}))})}else{const d=currentSelection();d.activities.forEach(x=>{const fresh=findAct(x.key)||x.item;entries.push({date:selected,weekday:fresh.weekday,type:"activity",start:fresh.start,end:fresh.end,title:fresh.title,en:fresh.en,venue:fresh.venue,item:fresh})});d.films.forEach(x=>entries.push({date:selected,weekday:x.item[1],type:"film",start:x.item[2].split("-")[0],end:x.item[2].split("-")[1],title:x.item[4],en:x.item[5],venue:x.item[3],unit:x.item[9],status:x.item[6],item:x.item}))}entries.sort((a,b)=>dayIndex(a.date)-dayIndex(b.date)||a.start.localeCompare(b.start));return entries}
