...
function findFilm(key){return SCHEDULE.find(x=>keyOf(x)===key)}function findAct(key){return MASTER_ACTIVITIES.find(x=>activityKey(x)===key)}function findSpecial(key){return SPECIAL_SESSIONS.find(x=>x.key===key)}
